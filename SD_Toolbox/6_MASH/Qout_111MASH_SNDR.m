...
%
% The routine works for the model mod3_1plus1plus1MASH. It allows to
...
%% Third Stage Modulator - Qout3:

psddata3 = spectrum.welch(psdset.win,psdset.framesize); % PSD Settings
dspdata3 = psd(psddata3,Qout3,'Fs',Fs); % Compute PSD

myfreqs3 = dspdata3.Frequencies; % Put the dc term at 1/2 bin so it can be 
myfreqs3(1) = myfreqs3(2)/2;     % plotted in the log scale of the graph
fb = Fs/(2*OSR);               % Ensure fb is updated from defaults

% PLOT PSD:

figure(3);
loglog(dspdata3.Frequencies, sqrt(dspdata3.Data), 'r', 'LineWidth', 1.5);
xlabel('Frequency (Hz)', 'FontWeight', 'Bold', 'FontSize', 10);
ylabel('Unit/sqrt (Hz)', 'FontWeight', 'Bold', 'FontSize', 10);
title('MOD Output Spectral Density', 'FontWeight', 'Bold', 'FontSize', 12);
grid on;
...
plot([10 fb], sqrt([0.95 0.95]), 'b--', 'LineWidth', 3);
plot([fb fb], sqrt([0.1*min(dspdata3.Data) 1]), 'b--', 'LineWidth', 3);
plot([fb Fs/2], sqrt([0.1*min(dspdata3.Data) 0.1*min(dspdata3.Data)]), ...
    'b--', 'LineWidth', 3);

text(1.2*fb, sqrt(min(dspdata3.Data)+2.5e-2), ...
    'Ideal LPF @ Fs/(2OSR)', 'FontSize', 12, 'FontWeight', 'Bold');
fb_text = sprintf('\n%.1e (Hz)', fb); % Label Passband Value
text(1.2*fb, sqrt(min(dspdata3.Data)), fb_text, 'FontSize', 12,...
    'FontWeight', 'Bold');
...
[sqnr3,sigpwr,bbpwr] = baseband_sqnr(Qout3, Fs, OSR, sinfreq, psdset);

enob3 = (20*log10(sqnr3)-1.76)/6.02; % In Bit
sqnr3 = 20*log10(sqnr3); % Convert SQN(+D)R in dB

% PLOT SQN(+D)R AND ENOB:

sqnr_text = sprintf('Simulated:\nSQN(+D)R = %.1f dB\nENOB = %.1f', sqnr3, enob3);
text(20, sqrt(max(dspdata3.Data)), sqnr_text, 'FontSize', 14, ...
    'FontWeight', 'Bold');
hold off;